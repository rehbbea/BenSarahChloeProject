...
<?php include 'template.php';?>
	...
	<h3> Your Bids</h3> 
		...
		date_expires>NOW() - INTERVAL 7 DAY AND a.auction_id IN (SELECT auctionid FROM t_bids WHERE buyer_id = " . $userid . ") ORDER BY(date_expires);";
		$currentq=$defaultq;
		?>	
      <div>
		<div class = "container"> 
		...
		<script>
         ...
         <?php
			...
			FROM (SELECT auctionid, max(`amount`) as currentval FROM `t_bids` GROUP BY auctionid) as h RIGHT JOIN t_auctions as a ON a.auction_id=h.auctionid, t_sellers as s, t_users as u, t_cat as c 
			where a.seller_id =s.user_id AND s.user_id=u.user_id AND a.cat= c.cat_id AND
			date_expires>NOW() - INTERVAL 7 DAY AND a.auction_id IN (SELECT auctionid FROM t_bids WHERE buyer_id = " . $userid . ") AND (lower(description) like lower('%" . $search . "%') OR lower(item_name) like lower('%" . $search . "%')) ORDER BY(date_expires);";
